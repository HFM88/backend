  ...
  user_register_endp.router.post('/register', async (req, res) => {
    ...
    try{
      let result = await collection['dbhelper.js'].conn.execute(
        'INSERT INTO `user_data`(`email`, `username`, `password` , `displayname` , `profilepic`) VALUES (?, ?, ?, ?,?)', 
        [req.body.email , req.body.username, await bcrypt.hash(req.body.password , 10) , req.body.username , 'user.png']
      );
